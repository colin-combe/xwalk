...
import xwalk.io.pdb.PDBwriter;
import xwalk.matter.parameter.Element;
import xwalk.matter.parameter.ParameterReader;
...
public class Atom {
    ...
    public Atom() {
        ...
        residueNumber = 0;
        iCode = ' ';
        xyz = new Point3d(0.0, 0.0, 0.0);
        occupancy = 0.0;
        temperaturFactor = 0.0;
        element = Element.CARBON;
        chargeState = 0;
        rank = 0;
        vanDerWaalsRadius = Constants.DEFAULT_ATOM_RADIUS;
        weight = 1;
        isAromatic = false;
        isMetalic = false;
    }
     ...
     * @param  flag
     *         - String object, which must start with "ATOM" or be "ATOM  " or
     *          "HETATM".
    ...
    public final boolean setFlag(final String flag) {
        if (!(flag.equalsIgnoreCase("ATOM  ")) && !(flag.equals("HETATM"))) {
            System.err.println("Warning PDB flag \"" + flag + "\" neither "
                             + "ATOM nor HETATM!");
            return false;
        } else if (flag.startsWith("ATOM")) {
            this.flag = "ATOM  ";
        } else {
            this.flag = flag;
        }
     ...
     * @see #setVanDerWaalsRadius(xwalk.constants.Constants.ParameterSets)
     ...
     * @return {@code TRUE} if atom belongs to an aromatic ring, {@FALSE}
     *         otherwise.
     * @see #setAromaticity(boolean)
     */
     ...
     * @return {@code TRUE} if atom has no belongs to an aromatic ring, {@FALSE}
     *         otherwise.
     * @see #setAromaticity(boolean)
     */
